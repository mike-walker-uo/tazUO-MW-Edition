...
public sealed class TileArtLoader : UOFileLoader
{
    ...
    private bool LoadEntry(uint graphic, out TileArtInfo tileArtInfo)
    {
        ...
        ref UOFileIndex entry = ref GetValidRefEntry((int)graphic);
        if (entry.Length == 0)
            return false;
        ...
        try
        {
            ...
            byte[] readData = _file.ReadArray(entry.Length);
            readData.CopyTo(bufSpan);
            ...
            StackDataReader reader = new StackDataReader(dbufSpan);
    ...
    public override Task Load()
    {
        return Task.Run(() =>
        {
            string path = UOFileManager.GetUOFilePath("tileart.uop");
            if (!File.Exists(path))
                return;

            _file = new UOFileUop(path, "build/tileart/{0:D8}.bin");
            Entries = new UOFileIndex[ushort.MaxValue + 0x4000];
