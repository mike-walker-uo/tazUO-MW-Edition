...
internal class ArtBrowserGump : Gump
{
    ...
    {
        int maxEntries = (Width / 100) * (Height / 100);
        resizableStaticPics = new ResizableStaticPic[maxEntries];
        for (int i = 0; i < maxEntries; i++)
        {
            resizableStaticPics[i] = new ResizableStaticPic(0, 100, 100)
            { CanMove = true, DrawBorder = true };
            resizableStaticPics[i].MouseDoubleClick += OnDoubleClick;
            dataBox.Add(resizableStaticPics[i]);
        }
