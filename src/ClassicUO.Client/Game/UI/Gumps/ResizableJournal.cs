...
namespace ClassicUO.Game.UI.Gumps
{
    internal class ResizableJournal : ResizableGump
    {
        ...
        private const int SEARCH_HEIGHT = 22;
        #region TABS
        ...
        private AlphaBlendControl _searchBackground;
        ...
        public ResizableJournal() : base(_lastWidth, _lastHeight, MIN_WIDTH, MIN_HEIGHT, 0, 0)
        {
            ...
            _scrollBarBase = new ScrollBar(
                ...
                BORDER_WIDTH + TAB_HEIGHT + SEARCH_HEIGHT,
                Height - TAB_HEIGHT - SEARCH_HEIGHT - (BORDER_WIDTH * 2));

            _journalArea = new JournalEntriesContainer(
                BORDER_WIDTH,
                BORDER_WIDTH + TAB_HEIGHT + SEARCH_HEIGHT,
                Width - SCROLL_BAR_WIDTH - (BORDER_WIDTH * 2),
                Height - (BORDER_WIDTH * 2) - TAB_HEIGHT - SEARCH_HEIGHT,
                _scrollBarBase,
                this);
            ...
            Add(_searchLabel = new Label("Find:", true, CustomGumpThemeManager.DataTextHue, font: 1));
            _searchBox = new JournalSearchBox(this, 0xFF, 64, 140, true, FontStyle.None, CustomGumpThemeManager.DataTextHue)
            {
                Width = 140,
                Height = 18
            };
            _searchBackground = new AlphaBlendControl(0.65f)
            {
                Width = _searchBox.Width,
                Height = _searchBox.Height
            };
            CustomGumpThemeManager.ApplyInputSurface(_searchBackground, 0.65f);
            _searchBox.Add(_searchBackground);
            _searchBox.SetTooltip("Filter journal text. Press Escape to clear.");
            Add(_searchBox);
        ...
        private void AddTab(string Name, MessageType[] filters)
        {
            ...
            CustomGumpThemeManager.StyleDataButton(nb);
        ...
        private void ApplyTheme()
        {
            ...
            CustomGumpThemeManager.StyleDataButton(_newTabButton);
            _searchLabel.Hue = CustomGumpThemeManager.DataTextHue;
            _searchBox.Hue = CustomGumpThemeManager.DataTextHue;
            CustomGumpThemeManager.ApplyInputSurface(_searchBackground, 0.65f);

            foreach (NiceButton tab in _tab)
            {
                CustomGumpThemeManager.StyleDataButton(tab);
            }
        }
        ...
        {
            _journalArea.SearchText = filter;
            _journalArea.CalculateScrollBarMaxValue();
            _scrollBarBase.Value = _scrollBarBase.MaxValue;
        }

        private sealed class JournalSearchBox : StbTextBox
        {
            private readonly ResizableJournal _owner;
            private string _last = string.Empty;

            public JournalSearchBox(
                ResizableJournal owner,
                byte font,
                int maxChars,
                int maxWidth,
                bool unicode,
                ...
                : base(font, maxChars, maxWidth, unicode, style, hue)
            {
                ...
                TextChanged += (s, e) =>
                {
                    ...
                    {
                        _last = current;
                        _owner.OnSearchChanged(current);
                    }
                };
            }
        ...
        private class JournalEntriesContainer : Control
        {
            ...
            public override bool Draw(UltimaBatcher2D batcher, int x, int y)
            {
                ...
                {
                    int scrollTop = _scrollBar.Value;
                    int scrollBottom = scrollTop + _scrollBar.Height;
                    foreach (JournalData journalEntry in journalDatas)
                    {
                        ...
                        if (!CanBeDrawn(journalEntry))
                            continue;

                        int relTop = my - y;
                        int relBottom = relTop + journalEntry.EntryText.Height;

                        // Past the bottom of the viewport — remaining entries are below; nothing more to draw.
                        if (relTop > scrollBottom)
                            break;

                        // Above the viewport — skip without drawing.
                        if (relBottom >= scrollTop)
                        {
                            if (!hideTimestamp)
                                journalEntry.TimeStamp.Draw(batcher, x, my - scrollTop);
                            journalEntry.EntryText.Draw(batcher, hideTimestamp ? x : x + (journalEntry.TimeStamp.Width + 5), my - scrollTop);
                        }
            ...
            private bool CanBeDrawn(JournalData entry)
            {
                if (SearchText.Length != 0
                    && entry.RawText.IndexOf(SearchText, StringComparison.OrdinalIgnoreCase) < 0)
                {
                    return false;
                }

                TextType type = entry.TextType;
                MessageType messageType = entry.MessageType;
            ...
            {
                public JournalData(TextBox textBox, TextBox timeStamp, string rawText, TextType textType, MessageType messageType)
                {
                    EntryText = textBox;
                    TimeStamp = timeStamp;
