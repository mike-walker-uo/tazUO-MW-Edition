    ...
    public class DressAgentManager
    {
        ...
        private void RegisterCommands()
        {
            ...
            CommandManager.Register("dressagent", args =>
            {
                if (args.Length < 3)
                {
                    GameActions.Print("Usage: -dressagent <dress|undress> \"<config name>\"");
                    return;
                }
                ...
                if (config == null)
                {
                    GameActions.Print($"Dress config '{configName}' not found");
                    return;
                }
                ...
                switch (action)
                {
                    case "dress":
                        ...
                        GameActions.Print($"Dressing from config: {configName}");
                        break;
                    case "undress":
                        UndressFromConfig(config);
                        GameActions.Print($"Undressing from config: {configName}");
                        break;
                    default:
                        GameActions.Print("Usage: -dressagent <dress|undress> \"<config name>\"");
                        break;
        ...
        {
            return config.UndressBagSerial != 0 ? config.UndressBagSerial : World.Player?.FindItemByLayer(Layer.Backpack)?.Serial ?? 0;
        }
        ...
        private void UnequipLayers(List<byte> layers, DressConfig config)
        {
            ...
            {
                var currentlyEquipped = World.Player?.FindItemByLayer((Layer)layer);
